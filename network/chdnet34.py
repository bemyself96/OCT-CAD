...
class CHDNet(nn.Module):
    def __init__(self, opt, ratio=6):
        ...
        # self.mlpnet = MLPNet(opt.info_dim, self.feature_dim)
        self.clsnet = CLSNet(self.feature_dim * ratio, opt.num_classes)
        self.bert_fc = nn.Linear(768 * 3, self.feature_dim)

        self.att_1_2 = CrossAttNet(input_dim=256, num_heads=4)
        self.att_1_3 = CrossAttNet(input_dim=256, num_heads=4)
        self.att_2_1 = CrossAttNet(input_dim=256, num_heads=4)
        self.att_2_3 = CrossAttNet(input_dim=256, num_heads=4)
        self.att_3_1 = CrossAttNet(input_dim=256, num_heads=4)
        self.att_3_2 = CrossAttNet(input_dim=256, num_heads=4)

        if opt.pre_train:
            self.load_pre_model(self.resnet_oct, opt.pre_model_path)
            self.load_pre_model(self.resnet_octa, opt.pre_model_path)
            self.load_pre_model(self.resnet_octp, opt.pre_model_path)
    ...
    def forward(self, img_oct, img_octa, img_octp, info_data, cls=True):
        oct = self.resnet_oct(img_oct)  # b,12
        octa = self.resnet_octa(img_octa)  # b,128
        oct_octa = torch.cat((oct, octa), dim=1)  # b,256
        octp = self.resnet_octp(img_octp)  # b,256
        layer = self.bert_fc(info_data)  # b,256
        ...
        output = self.clsnet(torch.cat((att_1, att_2, att_3), dim=1))

        return att_1, att_2, att_3, output
